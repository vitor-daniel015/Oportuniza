...
export function ContactInfoCard({ icon: Icon, title, description }: ContactInfoCardProps) {
  return (
    <div className="flex min-w-0 flex-col items-center px-0.5 sm:px-2">
      ...
      <h3 className="text-[9px] font-bold uppercase sm:text-sm lg:text-lg">{title}</h3>
      {description && (
        <p className="mt-1 max-w-full px-0.5 text-[8px] leading-tight sm:px-1 sm:text-sm lg:text-base">{description}</p>
      )}
    </div>
  );
}
