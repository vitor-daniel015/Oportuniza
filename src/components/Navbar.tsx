...
const links = [
  ...
  },
  {
    ...
    description: "Entenda como usar o Oportuniza",
    href: "/#servicos",
    icon: CircleHelp,
  },
  {
    ...
    description: "Veja quem pode realizar seu serviço",
    href: "/prestadores",
    icon: Search,
  },
  {
    label: "Fale conosco",
    shortLabel: "Contato",
    description: "Peça ajuda ou envie uma mensagem",
    href: "/contato",
    icon: MessageCircle,
  },
];
...
export function Navbar({ onAuthClick }: NavbarProps) {
  ...
  return (
    <>
      <header
        className={`relative z-40 bg-linear-to-r from-blue-depth via-[#27737C] to-green-sprout px-5 sm:px-8 md:h-40 md:rounded-bl-[110px] md:px-10 md:pt-7 lg:px-[7%] ${isGuest ? "h-55" : "h-38"}`}
      >
        <div
          className={`flex items-center justify-between md:block md:h-auto ${isGuest ? "h-34" : "h-full"}`}
        >
          <img
        ...
        <nav className="absolute left-1/2 top-29 hidden w-[86%] max-w-277 -translate-x-1/2 items-center justify-between rounded-[5px] bg-[#F7F9FA] px-5 py-4 shadow-[0_4px_4px_rgba(0,0,0,.25)] md:flex">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <Link
                to="/meu-perfil"
                className="flex items-center gap-2 rounded-lg bg-blue-depth px-5 py-3 text-sm font-bold text-white transition hover:bg-green-sprout"
              >
        ...
        >
          ...
          >
            ...
            {user ? (
              <div className="mt-auto space-y-3">
                ...
                >
                  <span className="flex items-center justify-center gap-2">
                    <BriefcaseBusiness size={19} /> Encontrar um profissional
                  </span>
                </a>
                <Link
                  to="/meu-perfil"
              ...
              >
                <span className="flex items-center justify-center gap-2">
                  <LogIn size={19} /> Entrar ou criar conta
                </span>
              </button>
            )}
          </aside>
